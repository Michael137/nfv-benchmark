...
void benchmark_loop( struct dataplane_port_t* port, struct pipeline_t* pipe,
                     struct rx_packet_stream* stream, uint16_t record_time,
...
struct pipeline_t* pipeline_build( struct rx_packet_stream* stream )
{
	...
	pipeline_element_add(
	    pipe, el_routing_create_with_file( MOD_BUFFER_SIZE_1,
	...
	pipeline_element_add( pipe,
	                      el_drop_mbuf_create( MOD_BUFFER_SIZE_3, stream ) );
	return pipe;
}
                     ...
                     uint64_t num_packets )
{
	...
	while( 1 )
	{
		...
		packet_send( port, fll_pkt );
		rte_eth_tx_buffer_flush( port->port_id, port->queue_id,
		                         port->tx_buffer );
...
int rxer( void* arg )
{
	...
	if( rx_stream_create( LARGEST_BUFFER, rte_eth_dev_socket_id( port_id ),
	                      &stream )
	    < 0 )
		...
		if( console_status == CONSOLE_PRINT )
		{
			...
			if( g_stream )
			{
				...
				printf( "Average queue_length %.2f\n",
				        (double)( st->average_queue_length )
				            / ( st->packet_count + 1 ) );
				// printf("Ring size: %d out of %d\n",
				//         rte_ring_get_capacity(st->ring) -
				//         rte_ring_free_count(st->ring),
				//         rte_ring_get_capacity(st->ring));
