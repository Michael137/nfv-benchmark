...
int sample_input() { return rand() % I; }
...
double drand( double high ) { return ( rand() * high ) / RAND_MAX; }
...
void packets_pool_uniform( struct packet_pool_t* pool )
{
	...
	for( packet_t* pkt = packets_pool_first( pool ); pkt < pool->end;
	     pkt           = (packet_t*)( pkt->data + pool->size ) )
	{
		if( pkt_first == NULL )
		{
			pkt_first = pkt;
...
int pipeline_adaptive( int m, int n, char const* name )
{
	...
	struct packet_pool_t* pool_random
	    = packets_pool_create( packet_count, PACKET_SIZE );
	struct packet_pool_t* pool_zipf
	    = packets_pool_create( packet_count, PACKET_SIZE );
	struct packet_pool_t* pool_uniform
	    = packets_pool_create( packet_count, PACKET_SIZE );
	struct packet_pool_t* pool_poisson
	    = packets_pool_create( packet_count, PACKET_SIZE );
	packets_pool_zipfian(pool_zipf, 0, packet_count - 1, 26, 8, 0.5);

	poisson_init();
	packets_pool_poisson( pool_poisson, 0, packet_count - 1, 26, 8, 0.5);
	poisson_fini();

	packets_pool_uniform( pool_uniform );
	// TODO: add poisson here
	...
	for( int i = 0; i < n; i++ )
	{
		...
		asm volatile( "mfence" ::: "memory" );
		int r = (int)( 100 * (float)( rte_get_tsc_cycles() - cycles )
		               / (float)( packet_count * repeat ) );
	...
	packets_pool_delete( &pool_poisson );
...
int datapath_init( int argc, char** argv, struct dataplane_port_t** port )
{
	*port   = 0;
	...
	log_info_fmt( "Num available dpdk ports (i.e., number of usable ethernet "
	              "devices): %d",
	              rte_eth_dev_count() );
