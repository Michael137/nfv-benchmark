...
struct merged_fastpass_t* merged_fastpass_create( uint32_t a, uint32_t b,
                                                  uint32_t c )
{
...
void merged_fastpass_process( struct element_t* ele, struct packet_t** pkts,
                              packet_index_t size )
{
...
//                               (ip.src == a) ||
			...
			uint32_t is_fast
			    = ( ip.src == a ) && ( ip.dst == b ) && ( ip.srcdst_port == c );
			sidx -= ( is_fast );
